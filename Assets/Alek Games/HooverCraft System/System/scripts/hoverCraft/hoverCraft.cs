...
namespace AlekGames.HoverCraftSystem.Systems.Main
{
    ...
    public class hoverCraft : MonoBehaviour
    {
        ...
        [SerializeField,
         Tooltip(
             "what input should the script use? axes - use axes other - asighn input from other script by function parseInput")]
        private InputT inputType;

        [SerializeField,
         Tooltip(
             "if should, just like a car have inverted input when going backwards. if set to input will invert input if vehicle wants to go back (back input is true), if velocity, will check if going back at speed over than 0.1, if non, will never invert input")]
        ...
        [Header("forward forces")] [SerializeField, Tooltip("forward of the vehicle")]
        ...
        [SerializeField,
         Tooltip(
             "independent - will mach input. dependent - will mach input if moving, will take velocity in consideration. instantDependent - will apply input only if moving (only if input to move forward backwards)")]
        private turnT turnType;

        [SerializeField] private bool canAirTurn = true;
        ...
        [SerializeField,
         Tooltip(
             "if script should clamp the velocity on xz axis to make counter forces be more accurate, but may make speedy movement harder")]
        ...
        [SerializeField,
         Tooltip(
             "if script should clamp the velocity on y axis to make counter forces be more accurate, but may make speedy movement harder")]
        ...
        [SerializeField,
         Tooltip(
             "a point from which the ground will be detected. used only if groundDetectionType is set to both or detector")]
        ...
        [Range(0.1f, 180)]
        private float maxNormalAngle = 90;

        [SerializeField,
         Tooltip(
             "force used to push veicle of hill with slope over maxNormal angle. Keep in mind that tis force is checked and possibly added foreach hoverPoint")]
        ...
        [Header("Jumping")] [SerializeField, Tooltip("the force of jump")]
        ...
        [Space] [SerializeField] private hoverPointSettings[] hoverPoints;
                ...
                switch (turnType)
                {
                    case turnT.independent:
                        ...
                        if (inputInvertType != invertType.none)
                        {
                            ...
                            else if (inputInvertType == invertType.velocity &&
                                     transform.InverseTransformDirection(rb.velocity).z < 0.1f) correctedInput.x *= -1;
                        }
                    ...
                    case turnT.dependent:
                        ...
                        if (inputInvertType != invertType.none)
                        {
                            if (inputInvertType == invertType.input && inputMovement.y == -1) mult *= -1;
                                     ...
                                     transform.InverseTransformDirection(rb.velocity).z < 0.1f) mult *= -1;
                        }
        ...
        {
            RaycastHit info;
            if (groundCheckType != groundCheckT.worldDown && Physics.Raycast(groundDetector.position,
                -groundDetector.up, out info, groundDetectionDistance, groundLayer))
            {
                grounded = true;
                groundNormal = info.normal;
            }
            else if (groundCheckType != groundCheckT.detectorDown && Physics.Raycast(groundDetector.position,
                Vector3.down, out info, groundDetectionDistance, groundLayer))
            {
                grounded = true;
                groundNormal = info.normal;
            }
        ...
        private void addMoveForces()
        {
            ...
            if (grounded && additionalGroundedForceUp != 0)
                rb.AddForce(Vector3.up * additionalGroundedForceUp * Time.deltaTime);
            else if (!grounded && additionalUnGroundedForceUp != 0)
                rb.AddForce(Vector3.up * additionalUnGroundedForceUp * Time.deltaTime);
        }
        ...
        private void addHoverForce()
        {
            ...
            {
                Vector3 levelingPos = hoverPoints[i].point.position +
                                      hoverPoints[i].point.up * hoverPoints[i].levelingOffset;

                if (Physics.Raycast(hoverPoints[i].point.position, -hoverPoints[i].point.up, out RaycastHit hitInfo,
                    hoverHeight, groundLayer))
                {
                    Vector3 updir = (liftTypeSpace == liftType.world) ? Vector3.up : transform.up;

                    rb.AddForceAtPosition(
                        updir * hoverPoints[i].liftForce * (1 - (hitInfo.distance / hoverHeight)) * Time.deltaTime,
                        hoverPoints[i].point.position, hoverPoints[i].liftForceMode);


                    if (Vector3.Angle(hitInfo.normal, Vector3.up) > maxNormalAngle)
                    {
                        Vector3
                            dir = new Vector3(hitInfo.normal.x, 0, hitInfo.normal.z)
                                .normalized; //just get the XZ dir, when there is Y the hoverCraft chitters to much, due to it moving up and down
                        rb.AddForce(dir * normalFixForce * Time.deltaTime,
                            normalFixForceMode); // just addForce, not at position. if at position hoverCraft will spin too much
                    }
                }
                else if ((hoverPoints[i].WhenToLevel == hoverPointSettings.lw.WhenVeicleGroundedAndThisNot &&
                          grounded) || (hoverPoints[i].WhenToLevel == hoverPointSettings.lw.whenThisNotGrounded &&
                                        !grounded))
                {
        ...
        {
            float multiplyier = 1;
            if (Vector3.Distance(Vector3.down, forwardVector.up) < Vector3.Distance(Vector3.up, forwardVector.up))
                multiplyier = -1;

            if (hoverPoints[index].point.position.y - hoverPoints[index].levelingHeightDiffrenceTolerance >
                center.position.y)
            {
                rb.AddForceAtPosition(Vector3.down * hoverPoints[index].levelingForce * multiplyier * Time.deltaTime,
                    levelingPos, hoverPoints[index].levelForceMode);
            }
            else if (hoverPoints[index].point.position.y + hoverPoints[index].levelingHeightDiffrenceTolerance <
                     center.position.y)
            {
                rb.AddForceAtPosition(Vector3.up * hoverPoints[index].levelingForce * multiplyier * Time.deltaTime,
                    levelingPos, hoverPoints[index].levelForceMode);
            }
        }
        ...
        public void findHoverPointFromPoint0()
        {
            ...
            for (int i = 0;
                i < hoverPoints.Length;
                i++) //paste settings from 1 to others, andd after assighn apropporiate child
            {
        ...
        public void setup(Transform center, Transform forward, Transform groundDetector, Transform hoverPoint0,
            LayerMask ground, bool useExternalInput)
        {
            this.center = center;
        ...
        public struct hoverPointSettings
        {
            ...
            [Tooltip("the force of leveling")] public float levelingForce;
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            for (int i = 0; i < hoverPoints.Length; i++)
            {
                ...
                if (Physics.Raycast(pos, down, out info, hoverHeight, groundLayer, QueryTriggerInteraction.Ignore))
                {
                    ...
                    GradientColorKey[] k =
                    {
                        new GradientColorKey(Color.red, 0), new GradientColorKey(new Color32(255, 215, 0, 1), 0.8f),
